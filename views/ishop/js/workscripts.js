$(document).ready(function(){
    ...
    $(".grid_list").click(function(){
        ...
        if(display == $.cookie("display")){
            // если значение (переключателя) совпадает с кукой - ничего не делаем
            return false;   
        }else{
    ...
    $("#auth").click(function(e){
        e.preventDefault(); // отличие от return false в том, что это решение не заканчивает работу скрипта, а просто запрещает работу ссылок, кнопок и т.д. retirn false останавливет полностью работу скрипта
        var login = $("#login").val();
        var pass = $("#pass").val();
        var auth = $("#auth").val();
        $.ajax({
    ...
    $(".kolvo").each(function(){
        ...
        $(this).change(function(){
            var qty = $(this).val(); // количество перед пересчётом
